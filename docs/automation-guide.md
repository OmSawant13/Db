...
```bash
node api/crawl-efficient-json.mjs https://efficient.app/
```
...
```bash
OUTPUT_DIR=playwright/playwright-output \
MAX_PAGES=60 \
DELAY_MS=1000 \
node playwright/playwright-efficient-crawler.mjs https://efficient.app/
```
...
```bash
OUTPUT_DIR=api/site-json-output-chrome \
MAX_PAGES=60 \
DELAY_MS=1000 \
FETCH_MODE=chrome \
node api/crawl-efficient-json.mjs https://efficient.app/
...
```bash
OUTPUT_DIR=api/site-json-output \
MAX_PAGES=60 \
DELAY_MS=1000 \
node api/crawl-efficient-json.mjs https://efficient.app/
```
